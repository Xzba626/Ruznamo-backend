import { FormEvent, useEffect, useState } from 'react';
import { apiRequest } from '../api/client';
import { fetchLicenses } from '../api/admin';
import { ApiClientError } from '../api/client';
import type { Paginated } from '../api/types';

type LicenseRow = {
  id: string;
  keyPrefix: string;
  status: string;
  plan: { code: string; name: string };
  user: { id: string; displayName: string | null; email: string | null } | null;
  activationCount: number;
  expiresAt: string | null;
  createdAt: string;
};

export function LicensesPage() {
  const [data, setData] = useState<Paginated<LicenseRow> | null>(null);
  const [search, setSearch] = useState('');
  const [page, setPage] = useState(1);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [revoking, setRevoking] = useState<string | null>(null);

  function load() {
    setLoading(true);
    fetchLicenses(page, search)
      .then((res) => setData(res as Paginated<LicenseRow>))
      .catch((err) => setError(err instanceof ApiClientError ? err.message : 'Failed to load licenses'))
      .finally(() => setLoading(false));
  }

  useEffect(() => {
    load();
  }, [page, search]);

  async function revoke(id: string) {
    if (!confirm('Revoke this license?')) return;
    setRevoking(id);
    try {
      await apiRequest(`/api/v1/admin/licenses/${id}/revoke`, { method: 'PATCH', body: JSON.stringify({}) });
      load();
    } catch (err) {
      setError(err instanceof ApiClientError ? err.message : 'Failed to revoke license');
    } finally {
      setRevoking(null);
    }
  }

  function onSearch(e: FormEvent) {
    e.preventDefault();
    setPage(1);
  }

  return (
    <div>
      <h1>Licenses</h1>
      <form className="toolbar" onSubmit={onSearch}>
        <input placeholder="Search licenses…" value={search} onChange={(e) => setSearch(e.target.value)} />
        <button type="submit" className="btn-secondary">Search</button>
      </form>
      {loading && <p>Loading licenses…</p>}
      {error && <div className="alert error">{error}</div>}
      {!loading && data && data.items.length === 0 && <p className="muted">No licenses found.</p>}
      {!loading && data && data.items.length > 0 && (
        <>
          <div className="table-wrap">
            <table>
              <thead>
                <tr>
                  <th>Key prefix</th>
                  <th>Plan</th>
                  <th>User</th>
                  <th>Status</th>
                  <th>Activations</th>
                  <th>Expires</th>
                  <th></th>
                </tr>
              </thead>
              <tbody>
                {data.items.map((license) => (
                  <tr key={license.id}>
                    <td className="mono">{license.keyPrefix}</td>
                    <td>{license.plan.name}</td>
                    <td>{license.user?.displayName ?? license.user?.email ?? '—'}</td>
                    <td>{license.status}</td>
                    <td>{license.activationCount}</td>
                    <td>{license.expiresAt ? new Date(license.expiresAt).toLocaleDateString() : '—'}</td>
                    <td>
                      {license.status !== 'REVOKED' && (
                        <button type="button" className="btn-danger" disabled={revoking === license.id} onClick={() => void revoke(license.id)}>
                          {revoking === license.id ? 'Revoking…' : 'Revoke'}
                        </button>
                      )}
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <div className="pager">
            <button type="button" className="btn-secondary" disabled={page <= 1} onClick={() => setPage((p) => p - 1)}>Previous</button>
            <span>Page {data.meta.page} of {data.meta.totalPages}</span>
            <button type="button" className="btn-secondary" disabled={page >= data.meta.totalPages} onClick={() => setPage((p) => p + 1)}>Next</button>
          </div>
        </>
      )}
    </div>
  );
}
